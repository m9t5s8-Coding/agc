...
AG_scope<Statements> LetStatement::ParseLet(Parser& parser, TokenName token) {
  ...
  switch (parser.peek_token_name()) {  // TODO Use Template Rather than hardcode
    case TokenName::AG_STRING: {
      return LetString(parser, id_position);
    }
    case TokenName::AG_INT: {
      return nullptr;
    }
...
void LetStatement::generate(CodeGenContext& context) {}

AG_scope<Statements> LetStatement::LetString(Parser& parser, size_t position) {
  parser.consume_token();
  auto variable_name = string_view_2_string(parser.get_token(position).value);
